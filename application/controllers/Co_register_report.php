...
class Co_register_report  extends Action_controller{	
	...
	public function datacount_check(){
		...
		for($i=0;$i<=(int)$filter_count;$i++){
			...
			if(($db_cond) && ($db_value)){
				...
				if((int)$table_name === 1){ $fliter_query .= ' and cw_co_register.'. $db_name ." ". $db_cond .' "'.$search_val.'"'; }
			}			
		}

		$co_reg_qry = 'select count(*) as rlst_count from cw_co_register inner join cw_uspm on cw_uspm.prime_uspm_id = cw_co_register.uspm inner join cw_project_and_drawing_master on cw_project_and_drawing_master.prime_project_and_drawing_master_id = cw_co_register.rdd_no inner join cw_client on cw_client.prime_client_id = cw_co_register.client_name inner join cw_project_and_drawing_master_drawings on FIND_IN_SET(cw_project_and_drawing_master_drawings.prime_project_and_drawing_master_drawings_id,cw_co_register.drawing_no) where cw_co_register.trans_status = 1 '.$fliter_query.' group by cw_co_register.prime_co_register_id order by cw_co_register.prime_co_register_id';
	...
	public function excel_export($process_month,$fliter_label,$fliter_type,$field_type,$filter_cond,$fliter_val,$multipick_val){
		...
		$team_qry = 'select GROUP_CONCAT(cw_team.team_name) as team_name,cw_team.prime_team_id,cw_co_register.team from cw_co_register inner join cw_uspm on cw_uspm.prime_uspm_id = cw_co_register.uspm inner join cw_project_and_drawing_master on cw_project_and_drawing_master.prime_project_and_drawing_master_id = cw_co_register.rdd_no inner join cw_client on cw_client.prime_client_id = cw_co_register.client_name inner join cw_team on FIND_IN_SET(cw_team.prime_team_id,cw_co_register.team) where cw_co_register.trans_status = 1 group by cw_co_register.prime_co_register_id order by cw_co_register.prime_co_register_id';
		...
		$revision_qry = 'select cw_co_register.co_number,cw_co_register.prime_co_register_id from cw_time_sheet inner join cw_time_sheet_time_line on cw_time_sheet_time_line.prime_time_sheet_id = cw_time_sheet.prime_time_sheet_id inner join cw_co_register on cw_co_register.prime_co_register_id = cw_time_sheet_time_line.co_number where cw_time_sheet.trans_status = 1 and cw_time_sheet_time_line.trans_status = 1';
		...
		foreach($co_reg_result as $key => $co_register_sheet){
			...
			$co_reg_sheet_value['I'] 		 = "Revision Hours";
			$co_reg_sheet_value['J']		 = "Estimation hours";
			$co_reg_sheet_value['K'] 		 = "Submitted date";
