...
class Mail_configurations  extends Base_controller{
	
	public function __construct(){
		parent::__construct('mail_configurations');
	...
	public function save(){
		...
		foreach($this->form_info as $setting){
			...
			if($label_id === 'mail_status'){
				$mail_status = trim($this->input->post($label_id));
			}
			if(($input_view_type === 1) || ($input_view_type === 2)){
		...
		if(count($can_process) > 0){
			...
			if((int)$form_id === 0){
				...
				$insert_info->next_result();
				if((int)$mail_status === 1){
					$this->update_active_mail($insert_id);
				}
				...
				if((int)$mail_status === 1){
					$this->update_active_mail($form_id);
				}
	...
	//UPDATE MAIL CONFIGURATIONS FOR ACTIVE
	public function update_active_mail($form_id){
		$created_on    = date("Y-m-d h:i:s");
		$set_query     = 'mail_status = "2" ,trans_updated_by = "'. $this->logged_id .'",trans_updated_date = "'.$created_on.'"';
		$update_query  = 'UPDATE '.$this->prime_table .' SET '. $set_query .' WHERE '. $this->prime_id .' != "'. $form_id .'"';
		$this->db->query("CALL sp_a_run ('UPDATE','$update_query')");
	}
}
?>