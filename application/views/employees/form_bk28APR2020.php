	...
	foreach($form_info as $setting){
		...
		if($prime_form_view_id === $input_for){
			...
			if((int)$field_type === 5){
				...
				$form_dropdown =  form_dropdown(array("name" => $label_id,"id" => $label_id,"class" =>'form-control input-sm select2'),$option_data,$input_value);
...
$(document).ready(function(){
	...
	if(user_role !== '1'){
		$('#offer_reference_no,#emp_name,#employee_code,#role,#date_of_joining').attr('readonly','readonly');
	}
	if(drop_exist === "1"){
		$(function(){
			$('.select2').select2({
	...
	$("#separation_type").change(function(){
		...
		//if(separation_type === '1' || separation_type === '3'){
	...
	$('#resignation_date').on("dp.hide",function (e) {
		...
		if(resignation_date){
			$.ajax({
				...
				data:{role:role,resignation_date:resignation_date},
				success: function(data) {
					var rslt = JSON.parse(data);
					if(rslt.success){
						toastr.success(rslt.message);
	...
	$('#last_working_date').on("dp.hide",function (e) {
		...
		if(last_date < resign_date){
			toastr.error("Seperation date not less than resigning date, please choose another date?");
			$('#last_working_date').val('');
		}
		if(last_working_date){
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("$controller_name/check_payroll"); ?>',
				data:{role:role,resignation_date:last_working_date},
				success: function(data) {
					var rslt = JSON.parse(data);
					if(rslt.success){
						toastr.success(rslt.message);
	...
	/*$('#stop_pay_month').on("dp.hide",function (e) {
		...
		var date_of_joining   = moment(doj, 'DD-MM-YYYY').format('YYYY-MM-DD');
		var stop_pay_month    = $('#stop_pay_month').val();
		var stop_pay          = moment(stop_pay_month, 'DD-MM-YYYY').format('YYYY-MM-DD');
		if(stop_pay <= date_of_joining){
			toastr.error("Stop pay month less than date of joining, please choose another date?");
			$('#stop_pay_month').val('');
		}
		if(today < stop_pay){
			toastr.error("Stop pay month less than today date, please choose another date?");
			$('#stop_pay_month').val('');
		}
	});*/
