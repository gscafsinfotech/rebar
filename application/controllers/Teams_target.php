	...
	public function select_team_employees(){
		...
		foreach($emp_result as $rslt){
			...
			$emp_name      = $rslt->emp_name;
		  	$table_body   .= "<tr>
								<td>$employee_code - $emp_name <input type='text' value='$employee_code' name='sub_emp_code[]'></td>
								<td><input type='text' name='target_value[]'></td>
								<td><input type='text' name='target_unit[]'></td>
							  </tr>";
		}
		$table_body   	  .= "<tr>
		...
		$table_content     = "<div style='margin:20px;'><table class='table table-striped table-bordered' id='team_target'>
								<thead>
									$table_head
								</thead>
								<tbody>
