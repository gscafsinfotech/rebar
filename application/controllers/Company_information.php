...
class Company_information  extends Action_controller{	
	public function __construct(){
		parent::__construct('company_information');
	...
	public function save(){
		...
		if($primary_color && $secondary_color){
			if($primary_color === $secondary_color){
				$can_process[] = "Primary color and Secondary Color should not be same";
			}else{
				$this->change_theme($primary_color,$secondary_color);
			}
		}
	...
	//CHANGE THEME 
	public function change_theme($primary_color,$secondary_color){
		$primary_color   = $this->input->post('primary_color');
		$secondary_color = $this->input->post('secondary_color');
		$file_path     = base_url("dist/cafs_rms.css");
		$file_path_bk  = base_url("dist/cafs_rms_".date('Y-m-d').".css");
		//echo $file_path; die;
		copy($file_path,$file_path_bk);
		$file_css      = file_get_contents($file_path);
		$qry = 'select primary_color,secondary_color from cw_company_information where trans_status = 1';
		$qry_format    = $this->db->query("CALL sp_a_run ('SELECT','$qry')");
		$qry_result    = $qry_format->result();
		$qry_format->next_result();
		$pri_color = $qry_result[0]->primary_color;
		$sec_color = $qry_result[0]->secondary_color;
		$new_primary   = str_replace($pri_color, $primary_color, $file_css);
		$new_secondary = str_replace($sec_color, $secondary_color, $new_primary);
		file_put_contents('dist/cafs_rms.css', $new_secondary);
		return true;
	}
}
?>