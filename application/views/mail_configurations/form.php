...
foreach($view_info as $view){
	...
	foreach($form_info as $setting){
		...
		$read = '';
		if(((int)$edit_id !==0) && ((int)$edit_id !== -1)){
			if((int)$edit_read === 1){
				$read = 'readonly';
			}
		}
		...
		if($prime_form_view_id === $input_for){
			...
			if(((int)$field_type === 1) || ((int)$field_type === 2) || ((int)$field_type === 3) || ((int)$field_type === 4) || ((int)$field_type === 9)){
				...
				if((int)$field_type === 9){
					...
					$form_input   = form_input(array("name"=>$hidden_id, "id"=>$hidden_id,"value"=>$hidden_value,"placeholder"=>"Search ".$label_name,$read=>true, "class"=>"form-control input-sm"));
			...
			if((int)$field_type === 5){
				$drop_exist = true;
				$drop_down_array = array("name" => $label_id,"id" => $label_id,"class" =>'form-control input-sm select2','style'=>"width:100%");
				if($read){
			...
			if((int)$field_type === 7){
				...
				$drop_down_array = array("name" => $multi_name,"multiple id" => $label_id,"class" =>'form-control input-sm select2','style'=>"width:100%");
				if($read){
					$drop_down_array['readonly'] = 'readonly';
				}
			...
			if(((int)$input_view_type === 3) && ((int)$form_view->$prime_id === 0) && ((int)$view_count === 0)){
				$view_count++;
				$document_load_script .= "$('#$label_id').bind('keyup blur change', function(e) {
							e.preventDefault();
							if($(form_id).valid()){
								$(form_id).submit();
							}else{
