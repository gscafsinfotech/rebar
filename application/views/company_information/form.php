	...
	foreach($form_info as $setting){
		...
		if($prime_form_view_id === $input_for){
			...
			if((int)$field_type === 3){
				...
				$form_input = form_input(array("name"=>$label_id, "id"=>$label_id,"value"=>$input_value,"placeholder"=>$label_name, $read=>true, "class"=>"form-control input-sm $valid_class",'readonly'=>$style));
				$input_box .= "<div class='form-group'>$form_label $form_input</div>";
			}else
			//DATE
			if((int)$field_type === 4){
...
$(document).ready(function(){
	$('#primary_color,#secondary_color').colorpicker();
	...
	*/
	...
	?>
	/* LOAD SCRIPT AND CONDITION LOAD */
	$('#upload_company_logo').change(function(){
		var imgWidth = $('#upload_company_logo').width();
		var imgHeight = $('#upload_company_logo').height();
		if(imgWidth < 150 || imgHeight < 25){
			toastr.error('Your image is too small, it must be greater than or equal to 150x25');
			$('#upload_company_logo,#company_logo').val('');
			$("#table_company_logo").attr("style", "display:none");
		}
	});
});
