...
class Sms_compaign  extends Action_controller{	
	public function __construct(){
		parent::__construct('sms_compaign');
	...
	public function index(){
		...
		$designation_info   = $this->db->query("CALL sp_a_run ('SELECT','SELECT prime_designation_id,designation FROM `cw_designation` where trans_status = 1')");
		$designation_result = $designation_info->result();
		$designation_info->next_result();
		$designation_list[""] = "---- Select Designation ----";
		foreach($designation_result as $designaton_rslt){
			$designation_id   = $designaton_rslt->prime_designation_id;
			$designation_name = $designaton_rslt->designation;
			$designation_list[$designation_id] = $designation_name;
		}
		$data['designation_list'] = $designation_list;
		
		$portfolio_info   = $this->db->query("CALL sp_a_run ('SELECT','SELECT prime_portfolio_id,portfolio_name FROM `cw_portfolio` where trans_status = 1')");
		$portfolio_result = $portfolio_info->result();
		$portfolio_info->next_result();
		$portfolio_list[""] = "---- Select Portfolio ----";
		foreach($portfolio_result as $portfolio_rslt){
			$portfolio_id   = $portfolio_rslt->prime_portfolio_id;
			$portfolio_name = $portfolio_rslt->portfolio_name;
			$portfolio_list[$portfolio_id] = $portfolio_name;
		}
	...
	public function get_employees_mobile(){
		$designation = $this->input->post('designation');
		$emp_mobile_qry = 'select prime_employees_id,emp_name,mobile_number from cw_employees where designation = "'.$designation.'" and trans_status = 1';
		$emp_mobile_info   = $this->db->query("CALL sp_a_run ('SELECT','$emp_mobile_qry')");
		$emp_mobile_result = $emp_mobile_info->result();
		$emp_mobile_info->next_result();
		echo json_encode(array('success' => TRUE, 'emp_list' => $emp_mobile_result));
	}
	
	public function sms_send(){
		$designation       = $this->input->post('designation');
		$mobile_list       = $this->input->post('mobile_list');
		$sms_content       = $this->input->post('sms_content');
		$mobile_no         = implode(",",$mobile_list);
		if($mobile_no){
			$rslt          = $this->trigger_sms($mobile_no,$sms_content);
			$save_log_rslt = $this->save_sms_log($designation,$mobile_no,$sms_content);
			if((int)$rslt === 1){
				echo json_encode(array('success' =>TRUE,'msg' => "Successfully Message Sent"));
			}else{
				echo json_encode(array('success' =>FALSE,'msg' => "Sending Failed, Due to Server Problem"));
			}
		}else{
			echo json_encode(array('success' =>FALSE,'msg' => "Sending Failed, Due to Server Problem"));
		}
	...
	public function save_sms_log($designation,$mobile_no,$sms_content){
		$created_on        = date("Y-m-d h:i:s");
		$save_log_qry_key  = "designation,to_mobile,sms_txt,trans_created_by,trans_created_date";
		$save_log_value    = '"'.$designation.'","'.$mobile_no.'","'.$sms_content.'","'.$this->logged_id.'",'.'"'.$created_on.'"';
		$save_log_query = "insert into cw_sms_log ($save_log_qry_key) values ($save_log_value)";
		$save_log_info    = $this->db->query("CALL sp_a_run ('INSERT','$save_log_query')");
		$save_log_result  = $save_log_info->result();
		$save_log_info->next_result();
		return true;
	}
}
?>