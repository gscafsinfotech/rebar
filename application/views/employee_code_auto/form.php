...
$(document).ready(function(){
	...
	*/
	...
	?>
	/* LOAD SCRIPT AND CONDITION LOAD */
	$('#category,#prefix,#start_value').parent().hide();
	$('#category,#prefix,#start_value').addClass('ignore');
	
	//code type auto or manual updates in code generator
	var code_type ='<?php echo $form_view->code_type; ?>';
	hide_and_show(code_type);
...
//HIDE AND SHOW
function hide_and_show(code_type){
	if(parseInt(code_type) === 1){
		$('#category,#prefix,#start_value').parent().show();
		$('#category,#prefix,#start_value').removeClass('ignore');
	}else{
		$('#category,#prefix,#start_value').parent().hide();
		$('#category,#prefix,#start_value').addClass('ignore');
	}
