...
class Employee_code_auto  extends Action_controller{	
	public function __construct(){
		parent::__construct('employee_code_auto');
	...
	public function save(){
		...
		if(count($can_process) > 0){
			...
			$exist      = $this->check_exist($this->input->post('category'),$form_id);
			$exist_exp  = explode("/",$exist);
			if(((int)$exist_exp[0] === 0) || ((int)$exist_exp[0] === 1) && ((int)$form_id === (int)$exist_exp[2])){
				if((int)$form_id === 0){
				...
				echo json_encode(array('success' => FALSE, 'message' => $exist_exp[1]));
			}
		}
	}
	...
	public function check_exist($category,$form_id){
		$this->db->select('GROUP_CONCAT(prime_category_id) as roles');
		$this->db->from('category');
		$role_rslt = $this->db->get()->result();
		$roles = $role_rslt[0]->roles;
		if($category === "All"){
			$qry = 'category in ('.$roles.') and trans_status=1';
			$message = "Create For Every Roles";
		}else{
			$qry = 'category = "All" and trans_status=1';
			$message = "Already Created for All Roles";
		}
		$search_query   = 'select * from cw_employee_code_auto where '.$qry;
		...
		$id             = $result[0]->prime_employee_code_auto_id;
		return $num_rows."/".$message."/".$id;
	}
}
?>