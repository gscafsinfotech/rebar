...
<?php
if($_GET){
	$url_consultancy = $_GET['frm'];
	$url_consult_id  = $_GET['consultancy'];
	$consultancy_decode_id = base64_decode($url_consult_id);
}
	...
	foreach($form_info as $setting){
		...
		if($prime_form_view_id === $input_for){
			...
			if((int)$field_type === 5){
				...
				// print_r($form_view['all_pick']);

				if($url_consultancy === 'consultancy'){
					if((int)$prime_form_id===879){
					foreach($form_view['all_pick'][$prime_form_id] as $opt_key =>$opt_value){
						if((int)$opt_key ===2){
							...
							$option .= "<option value='$opt_key' $selected>$opt_value</option>";
						}
						
					}else
					if((int)$prime_form_id===904){
						foreach($form_view['all_pick'][$prime_form_id] as $opt_key =>$opt_value){
							if((int)$opt_key ===(int)$consultancy_decode_id){
								$selected = "selected";
							}else{
								$selected = "disabled";
							}
...
<html lang="en">
	...
	$('#percentage').on('keydown keyup', function(e){
		...
		$("#consultancy").change(function(){
			consultancy = $('#consultancy').val();
			hide_show_consultancy(consultancy);
		});
...
function hide_show_consultancy(consultancy){
	if(parseInt(consultancy) === 1){
		$('#recruiter').parent().show();
		$('#recruiter').removeClass('ignore');
	}else{
				...
				if(rslt.message === "completed"){
					...
					$.each(rslt.form_rslt, function(key,value){
						...
						if((value.field_type === "5") || (value.field_type === "7")){
							...
							if(key === "applied_by"){
								var url_consultancy = '<?php echo $url_consultancy; ?>';
									if(url_consultancy === 'consultancy'){
										value.input_value = 2;
										hide_show(value.input_value);
									}else{
							...
							if(key === "company_location"){
								var company_location = $('#company_location').val();
								company_location_function(company_location);
							}
							if(key === "consultancy"){
								var consultancy = $("#consultancy").val();
								 // var consultancy_decode_id = '<?php echo $consultancy_decode_id;?>';
								 //  if(parseInt(consultancy_decode_id) === 1){
								  	hide_show_consultancy(consultancy);
								  }


							// 	// var consultancy = '<?php echo $consultancy_decode_id; ?>';
							// 	// if(url_consultancy === 'Consultancy'){
							// 	// 	value.input_value = consultancy;
...
function default_hide_inputs(){
	$('#degree,#employee_code,#relived_date,#current_salary,#notice_period,#Recruiter').parent().hide();
	$('#degree,#employee_code,#relived_date,#current_salary,#notice_period,#email_id,#Recruiter').addClass('ignore');
	$('#email_id,#Recruiter').parent().find('label').removeClass('required');
}
...
function company_location_function(company_location){
	var send_url = 'enrolment_callback.php?frm=company_location_tab';
	$.ajax({
		type: "POST",
		url: send_url,
		data:{company_location:company_location},
		success: function(data){
			$('#company_branch').html(data);
		}
	});	
}
