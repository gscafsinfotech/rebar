<?php 
	...
	$payslip_url    = base_url("/payslip/staff/");
		...
		columns: [{
			...
			<?php
				...
				foreach($table_head as $table){
					...
					if(($field_type === 5)||($field_type === 9)){
						if($table->label_name === 'termination_status'){
							echo "{title:'$view_name',data: '$label_name',type: 'date',visible:true,
								render:function(value) {											
			...
			/*{title:'payslip',
				...
				render:function (value) {
					...
					old_pre.setMonth(old_pre.getMonth() - 2);
					now     = moment(now.toLocaleDateString("en-US", {timeZone: "Asia/Kolkata"})).format('MM-YYYY');
					pre     = moment(pre.toLocaleDateString("en-US", {timeZone: "Asia/Kolkata"})).format('MM-YYYY');
					old_pre = moment(old_pre.toLocaleDateString("en-US", {timeZone: "Asia/Kolkata"})).format('MM-YYYY');
					var now_url = '<?php echo $payslip_url; ?>'+now+'_payslip/'+value+'.pdf',
					pre_url = '<?php echo $payslip_url; ?>'+pre+'_payslip/'+value+'.pdf',
					old_pre_url = '<?php echo $payslip_url; ?>'+old_pre+'_payslip/'+value+'.pdf';
					return "<li class='dropdown'> <a class='btn btn-xs btn-primary dropdown-toggle' type='button' id='dropdownMenu2' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'> <i class='fa fa-file-pdf-o' aria-hidden='true'></i> Payslip </a> <ul class='dropdown-menu dropdown-menu-left' aria-labelledby='dropdownMenu2'> <li><a onclick='get_playslip(\""+now+"\",\""+value+"\")'> <i class='fa fa-angle-double-right fa-lg' aria-hidden='true'></i> "+now+"</a></li><li><a onclick='get_playslip(\""+pre+"\",\""+value+"\")'> <i class='fa fa-angle-double-right fa-lg' aria-hidden='true'></i> "+pre+"</a></li><li><a onclick='get_playslip(\""+old_pre+"\",\""+value+"\")'> <i class='fa fa-angle-double-right fa-lg' aria-hidden='true'></i> "+old_pre+"</a></li> </ul> </li>";
				}
			}*/
			/*
			{title:'Delete',
				...
				render:function (value) {
					if (value === null) return '';
					return "<button class='btn btn-xs btn-danger delete'>Delete</button>";
				}
			},*/
		],
	});
...
function get_playslip(month,emp_code){
   var send_url = '<?php echo site_url("$controller_name/check_file_exists"); ?>'; 
	$.ajax({
		type: "POST",
		url: send_url,
		data:{month:month,emp_code:emp_code},
		success: function(data) {
			var rslt = JSON.parse(data);
			if(rslt.success){
				window.open(rslt.url,'_blank');
			}else{
				toastr.warning('Payslip not generating');
			}
		}
	});
}
